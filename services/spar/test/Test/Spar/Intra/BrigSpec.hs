...
spec = do
  describe "toUserSSOId, fromUserSSOId" $ do
    ...
    it "example" $ do
      let have =
            UserRef
              (Issuer $ mkuri "http://social.network/")
              ( either (error . show) id $
                  mkNameID (mkUNameIDTransient "V") (Just "kati") (Just "rolli") (Just "jaan")
              )
          want =
            UserSSOId
              "<Issuer xmlns:samlp=\"urn:oasis:names:tc:SAML:2.0:protocol\" xmlns:samla=\"urn:oasis:names:tc:SAML:2.0:assertion\" xmlns:samlm=\"urn:oasis:names:tc:SAML:2.0:metadata\" xmlns:ds=\"http://www.w3.org/2000/09/xmldsig#\" xmlns=\"urn:oasis:names:tc:SAML:2.0:assertion\">http://social.network/</Issuer>"
