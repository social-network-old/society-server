    ...
    let usr :: Scim.User SparTag
        usr =
          Scim.User
            { Scim.schemas =
                [ Scim.User20,
                  Scim.CustomSchema "urn:society:scim:schemas:profile:1.0",
                  Scim.CustomSchema "urn:ietf:params:scim:schemas:extension:society:1.0:User"
                ],
      ...
      it "can add to rich info map" $ do
        let operationJSON =
              [aesonQQ|{
                                       ...
                                       "operations" : [{
                                         "op" : "add",
                                         "path" : "urn:ietf:params:scim:schemas:extension:society:1.0:User:newAttr",
                                         "value" : "newValue"
                                       }]
                                     }|]
      ...
      it "can replace in rich info map" $ do
        let operationJSON =
              [aesonQQ|{
                                       ...
                                       "operations" : [{
                                         "op" : "replace",
                                         "path" : "urn:ietf:params:scim:schemas:extension:society:1.0:User:oldAttr",
                                         "value" : "newValue"
                                       }]
                                     }|]
        let (Aeson.Success (PatchOp [operation])) = Aeson.parse (parseJSON @(PatchOp SparTag)) operationJSON
      ...
      it "adds new fields to rich info assoc list at the end" $ do
        let operationJSON =
              [aesonQQ|{
                                       ...
                                       "operations" : [{
                                         "op" : "add",
                                         "path" : "urn:society:scim:schemas:profile:1.0:newAttr",
                                         "value" : "newValue"
                                       }]
                                     }|]
        let (Aeson.Success (PatchOp [operation])) = Aeson.parse (parseJSON @(PatchOp SparTag)) operationJSON
      ...
      it "can replace in rich info assoc list while maintaining order" $ do
        let operationJSON =
              [aesonQQ|{
                                       ...
                                       "operations" : [{
                                         "op" : "replace",
                                         "path" : "urn:society:scim:schemas:profile:1.0:secondAttr",
                                         "value" : "newSecondVal"
                                       }]
                                     }|]
        let (Aeson.Success (PatchOp [operation])) = Aeson.parse (parseJSON @(PatchOp SparTag)) operationJSON
      ...
      it "can remove from rich info assoc list" $ do
        let operationJSON =
              [aesonQQ|{
                                       ...
                                       "operations" : [{
                                         "op" : "remove",
                                         "path" : "urn:society:scim:schemas:profile:1.0:oldAttr"
                                       }]
                                     }|]
        let (Aeson.Success (PatchOp [operation])) = Aeson.parse (parseJSON @(PatchOp SparTag)) operationJSON
        applyOperation (ScimUserExtra (RichInfo mempty [RichField "oldAttr" "oldValue"])) operation
