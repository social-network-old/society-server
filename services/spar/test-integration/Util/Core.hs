...
cliOptsParser =
  ...
  where
    defaultIntPath = "/etc/society/integration/integration.yaml"
    defaultSparPath = "/etc/society/spar/conf/spar.yaml"
...
-- URLs any more <https://github.com/social-network/society-server/pull/466#issuecomment-419396359>, we
-- removed the mock idp functionality.  if you want to re-introduce it,
-- <https://github.com/social-network/society-server/pull/466/commits/9c93f1e278500522a0565639140ac55dc21ee2d2>
...
-- 'UUID.nextRandom'.
nextSocietyId :: MonadIO m => m (Id a)
nextSocietyId = Id <$> liftIO UUID.nextRandom

nextSocietyIdP :: MonadIO m => m SocietyIdP
nextSocietyIdP = SocietyIdP <$> (Id <$> liftIO UUID.nextRandom) <*> pure [] <*> pure Nothing
...
makeTestIdP :: (HasCallStack, MonadRandom m, MonadIO m) => m (IdPConfig SocietyIdP)
makeTestIdP = IdPConfig <$> (IdPId <$> liftIO UUID.nextRandom) <*> (fst <$> makeTestIdPMetadata) <*> nextSocietyIdP
...
-- "social.network=; Path=/sso/finalize-login; Expires=Thu, 01-Jan-1970 00:00:00 GMT; Max-Age=-1; Secure"
...
tryLogin privkey idp userSubject = do
  ...
  liftIO $ do
    ...
    bdy `shouldContain` "<title>society:sso:success</title>"
...
loginSsoUserFirstTime idp privCreds = do
  ...
  let societyCookie = maybe (error (show sparAuthnResp)) id . lookup "Set-Cookie" $ responseHeaders sparAuthnResp
  accessResp :: ResponseLBS <-
    call $
      post ((env ^. teBrig) . path "/access" . header "Cookie" societyCookie . expect2xx)
  let uid :: UserId
