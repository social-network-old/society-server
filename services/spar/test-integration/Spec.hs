...
main :: IO ()
main = do
  (societyArgs, hspecArgs) <- partitionArgs <$> getArgs
  env <- withArgs societyArgs mkEnvFromOptions
...
partitionArgs = go [] []
  where
    go societyArgs hspecArgs ("-s" : x : xs) = go (societyArgs <> ["-s", x]) hspecArgs xs
    go societyArgs hspecArgs ("-i" : x : xs) = go (societyArgs <> ["-i", x]) hspecArgs xs
    go societyArgs hspecArgs (x : xs) = go societyArgs (hspecArgs <> [x]) xs
    go societyArgs hspecArgs [] = (societyArgs, hspecArgs)
