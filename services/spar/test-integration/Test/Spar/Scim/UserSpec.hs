...
specCreateUser = describe "POST /Users" $ do
  ...
  it "adds a Society scheme to the user record" $ testSchemaIsAdded
...
-- | Test that Society-specific schemas are added to the SCIM user record, even if the schemas
      ...
      (Aeson.Success patchOp) =
        fromJSON
          [aesonQQ|{
                                                      ...
                                                      "operations" : [{
                                                        "op" : "replace",
                                                        "path" : "urn:ietf:params:scim:schemas:extension:society:1.0:User:Platforms",
                                                        "value" : "Arch, BTW"
                                                      }]
                                                    }|]
      ...
      (Aeson.Success patchOp) =
        fromJSON
          [aesonQQ|{
                                                      ...
                                                      "operations" : [{
                                                        "op" : "replace",
                                                        "path" : "urn:society:scim:schemas:profile:1.0:Platforms",
                                                        "value" : "Arch, BTW"
                                                      }]
                                                    }|]
   in testRichInfo patchOp richInfo richInfoOverwritten richInfoPatched
    ...
    createViaSamlFails idp privCreds uref = do
      ...
      liftIO $ do
        maybe (error "no body") cs (responseBody resp)
          `shouldContain` "<title>society:sso:error:forbidden</title>"
...
testFindNonProvisionedUser = do
  ...
  Just brigUser <- runSpar $ Intra.getBrigUser member
  liftIO $ userManagedBy brigUser `shouldBe` ManagedBySociety
...
specPatchUser = do
  ...
  describe "PATCH /Users/:id" $ do
    ...
    it "displayName cannot be removed in spar (though possible in scim). Diplayname is required in Society" $ do
      pendingWith
        "We default to the externalId when displayName is removed. lets keep that for now"
    {-env <- ask
    (tok, _) <- registerIdPAndScimToken
...
-- it sends a userName that is not a valid society handle. So we should treat 'invalid' as 'not
-- found'.
