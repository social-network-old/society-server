      ...
      it "responds with status 200 and a valid html page with constant expected title." $ do
        ...
        liftIO $ do
          ...
          ('1', cs @LBS @String (Servant.errBody outcome))
            `shouldSatisfy` (("<title>society:sso:error:forbidden</title>" `List.isInfixOf`) . snd)
    ...
    context "granted" $ do
      it "responds with status 200 and a valid html page with constant expected title." $ do
        ...
        liftIO $ do
          ...
          ('1', cs @LBS @String (Servant.errBody outcome))
            `shouldSatisfy` (("<title>society:sso:success</title>" `List.isInfixOf`) . snd)
          ('2', XML.parseLBS XML.def (Servant.errBody outcome))
            `shouldSatisfy` (isRight . snd)
    ...
    context "denied" $ do
      it "responds with status 303 with appropriate details." $ do
        ...
        liftIO $ do
          ...
          uriScheme loc `shouldBe` (URI.Scheme "society")
...
mkAuthnReqMobile idpid = do
  env <- ask
  let succurl = [uri|society://login-granted/?cookie=$cookie&userid=$userid|]
      errurl = [uri|society://login-denied/?label=$label|]
