...
`/sso/metadata` will be requested by the IdPs to learn how to talk to society.

`/sso/initiate-login`, `/sso/finalize-login` are for the SAML authentication handshake performed by a user in order to log into society.  They are not exactly standard in their details: they may return HTML or XML; redirect to error URLs instead of throwing errors, etc.
...
Okta will ask you to provide two URLs when you set it up for talking to social-network:

1. The `Single sign on URL`.  This is the end-point that accepts the user's credentials after successful authentication against the IdP.  Choose `/sso/finalize-login` with schema and hostname of the society server you are configuring.
...
instance ToSchema SocietyIdP where
  declareNamedSchema = genericDeclareNamedSchema samlSchemaOptions
