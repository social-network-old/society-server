...
-- | Similar to 'Network.Society.Client.API.Auth.tokenResponse', but easier: we just need to set the
...
-- is not from the society client, but from a browser that is still processing a redirect from the
-- IdP.)
