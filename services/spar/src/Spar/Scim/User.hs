...
validateHandle txt = case parseHandle txt of
  ...
  Nothing ->
    throwError $
      Scim.badRequest
        Scim.InvalidValue
        (Just (txt <> "is not a valid Society handle"))
...
-- Brig. See <https://github.com/social-network/society-server/pull/559#discussion_r247466760>.
validateScimUser' ::
  forall m.
...
validateScimUser' idp richInfoLimit user = do
  ...
  where
    -- Validate a name (@displayName@). It has to conform to standard Society rules.
    ...
    validateName txt = case checkedEitherMsg @_ @1 @128 "displayName" txt of
      ...
      Left err ->
        throwError $
          Scim.badRequest
            Scim.InvalidValue
            (Just ("displayName must be a valid Society name, but: " <> Text.pack err))
...
updateValidScimUser tokinfo uid newScimUser = do
  ...
  -- <https://github.com/social-network/society-server/pull/559#discussion_r247392882>).
  --
...
assertUserRefNotUsedElsewhere userRef societyUserId = do
  mExistingUserId <- lift $ getUser userRef
  unless (mExistingUserId `elem` [Nothing, Just societyUserId]) $ do
...
--
-- TODO: use the same algorithm as Society clients use.
...
-- | Convert from the Society phone type to the SCIM phone type.
...
-- | Convert from the Society email type to the SCIM email type.
