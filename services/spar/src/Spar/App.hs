...
instance SPStoreIdP SparError Spar where
  type IdPConfigExtra Spar = SocietyIdP
...
-- then delete it on the team admin page in society.  If a user is deleted in society but not in the IdP,
...
-- the past.
--
-- FUTUREWORK: once we support <https://github.com/social-network/hscim scim>, brig will refuse to delete
  ...
  SAML.AccessGranted userref -> do
    uid :: UserId <- do
      ...
      case (viaBindCookie, viaSparCassandra, viaSparCassandraOldIssuer) of
        ...
        -- "reauthentication" branch, so we pass 'ManagedBySociety'.
        (Nothing, Nothing, Nothing) -> autoprovisionSamlUser userref Nothing ManagedBySociety
        ...
        (Just uid, Just uid', Nothing)
          ...
          | uid == uid' -> pure uid
          -- Attempt to use ssoid for a second Society user
...
-- - A title element with contents @society:sso:<outcome>@.  This is chosen to be easily parseable and
...
errorPage err inputs mcky =
  ...
  where
    ...
    errbody =
      [ "<head>",
        "  <title>society:sso:error:" <> cs (Wai.label werr) <> "</title>",
        "</head>",
        "</body>",
