...
import Galley.Types.Conversations.Roles (roleNameSocietyAdmin)
...
testBadFingerprint config db brig galley _cannon = do
  ...
  sref <- withSystemTempFile "society-provider.key" $ \fp h -> do
...
createConv g u us =
  ...
  where
    conv = NewConv (makeIdOpaque <$> us) Nothing Set.empty Nothing Nothing Nothing Nothing roleNameSocietyAdmin
...
wsAssertMemberJoin ws conv usr new = void $ liftIO
  ...
  $ \n -> do
    ...
    evtFrom e @?= usr
    evtData e @?= Just (EdMembersJoin (SimpleMembers (fmap (\u -> SimpleMember u roleNameSocietyAdmin) new)))
...
svcAssertMemberJoin buf usr new cnv = liftIO $ do
  ...
  case evt of
    Just (TestBotMessage e) -> do
      let msg = SimpleMembers $ fmap (\u -> SimpleMember u roleNameSocietyAdmin) new
...
testMessageBotUtil uid uc cid pid sid sref buf brig galley cannon = do
  ...
  liftIO $ do
    ...
    assertEqual "members" [OtherMember uid Nothing roleNameSocietyAdmin] (bcnv ^. Ext.botConvMembers)
