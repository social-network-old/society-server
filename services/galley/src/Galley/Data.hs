...
createConversation usr name acc role others tinfo mtimer recpt othersConversationRole = do
  ...
  mems <- snd <$> addMembersUncheckedWithRole now conv (usr, roleNameSocietyAdmin) (list1 (usr, roleNameSocietyAdmin) ((,othersConversationRole) <$> fromConvSize others))
...
addMembersUnchecked t conv orig usrs = addMembersUncheckedWithRole t conv (orig, roleNameSocietyAdmin) ((,roleNameSocietyAdmin) <$> usrs)
...
newMember = flip newMemberWithRole roleNameSocietyAdmin
      ...
      Just $
        Member
          { memId = usr,
            ...
            memHiddenRef = hidr,
            memConvRoleName = fromMaybe roleNameSocietyAdmin crn
          }
