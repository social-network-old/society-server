...
tests s =
  testGroup "Teams API" $
    [ test s "create team" testCreateTeam,
      ...
      test s "update conversation as member" (testUpdateTeamConv RoleMember roleNameSocietyAdmin),
      test s "update conversation as partner" (testUpdateTeamConv RoleExternalPartner roleNameSocietyMember),
...
testAddTeamConvLegacy = do
  ...
  WS.bracketRN c allUserIds $ \wss -> do
    ...
    mapM_ (assertConvMemberWithRole roleNameSocietyAdmin cid) allUserIds
  ensureQueueEmpty
...
testAddTeamConvWithRole = do
  ...
  WS.bracketRN c [owner, extern, mem1 ^. userId, mem2 ^. userId] $ \ws@[wsOwner, wsExtern, wsMem1, wsMem2] -> do
    ...
    -- Regular conversation:
    cid2 <- Util.createTeamConvWithRole owner tid [extern] (Just "blaa") Nothing Nothing roleNameSocietyAdmin
    checkConvCreateEvent cid2 wsOwner
    checkConvCreateEvent cid2 wsExtern
    mapM_ (assertConvMemberWithRole roleNameSocietyAdmin cid2) [owner, extern]
    -- Regular conversation (using member role for participants):
    cid3 <- Util.createTeamConvWithRole owner tid [extern] (Just "blaa") Nothing Nothing roleNameSocietyMember
    checkConvCreateEvent cid3 wsOwner
    checkConvCreateEvent cid3 wsExtern
    assertConvMemberWithRole roleNameSocietyAdmin cid3 owner
    assertConvMemberWithRole roleNameSocietyMember cid3 extern
...
testAddManagedConv = do
  ...
  let conv =
        NewConvManaged $
          NewConv [makeIdOpaque owner] (Just "blah") (Set.fromList []) Nothing (Just tinfo) Nothing Nothing roleNameSocietyAdmin
  post
    ( g
        . path "/conversations"
        . zUser owner
...
testFeatureFlags = do
  ...
  withCustomSearchFeature FeatureTeamSearchVisibilityDisabledByDefault $ do
    getTeamSearchVisibility tid2 TeamSearchVisibilityDisabled
    getTeamSearchVisibilityInternal tid2 TeamSearchVisibilityDisabled
    setTeamSearchVisibilityInternal tid2 TeamSearchVisibilityEnabled
    getTeamSearchVisibility tid2 TeamSearchVisibilityEnabled
    getTeamSearchVisibilityInternal tid2 TeamSearchVisibilityEnabled
    setTeamSearchVisibilityInternal tid2 TeamSearchVisibilityDisabled
    getTeamSearchVisibility tid2 TeamSearchVisibilityDisabled
    getTeamSearchVisibilityInternal tid2 TeamSearchVisibilityDisabled
  tid3 <- Util.createNonBindingTeam "foo" owner []
  withCustomSearchFeature FeatureTeamSearchVisibilityEnabledByDefault $ do
    getTeamSearchVisibility tid3 TeamSearchVisibilityEnabled
    getTeamSearchVisibilityInternal tid3 TeamSearchVisibilityEnabled
    setTeamSearchVisibilityInternal tid3 TeamSearchVisibilityDisabled
    getTeamSearchVisibility tid3 TeamSearchVisibilityDisabled
    getTeamSearchVisibilityInternal tid3 TeamSearchVisibilityDisabled
    setTeamSearchVisibilityInternal tid3 TeamSearchVisibilityEnabled
    getTeamSearchVisibility tid3 TeamSearchVisibilityEnabled
    getTeamSearchVisibilityInternal tid3 TeamSearchVisibilityEnabled
