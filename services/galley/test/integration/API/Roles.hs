...
handleConversationRoleAdmin = do
  ...
  cid <- WS.bracketR3 c alice bob chuck $ \(wsA, wsB, wsC) -> do
    ...
    postMembersWithRole alice (singleton jack) cid roleNameSocietyMember !!! const 200 === statusCode
    void . liftIO $ WS.assertMatchN (5 # Second) [wsA, wsB, wsC] $
      wsAssertMemberJoinWithRole cid alice [jack] roleNameSocietyMember
    return cid
  -- Added bob as a society_admin and do the checks
  societyAdminChecks cid alice bob jack
  -- Demote bob and run the member checks
  WS.bracketR3 c alice bob chuck $ \(wsA, wsB, wsC) -> do
    let updateDown = OtherMemberUpdate (Just roleNameSocietyMember)
    putOtherMember alice bob updateDown cid !!! assertActionSucceeded
    void . liftIO $ WS.assertMatchN (5 # Second) [wsA, wsB, wsC] $ do
      wsAssertMemberUpdateWithRole cid alice bob roleNameSocietyMember
  societyMemberChecks cid bob alice jack
...
handleConversationRoleMember = do
  ...
  let role = roleNameSocietyMember
  cid <- WS.bracketR3 c alice bob chuck $ \(wsA, wsB, wsC) -> do
    rsp <- postConvWithRole alice [bob, chuck] (Just "gossip") [] Nothing Nothing role
    void $ assertConvWithRole rsp RegularConv alice alice [bob, chuck] (Just "gossip") Nothing role
    let cid = decodeConvId rsp
    ...
    void . liftIO $ WS.assertMatchN (5 # Second) [wsA, wsB, wsC] $
      wsAssertMemberJoinWithRole cid alice [eve] role
    return cid
  -- Added bob as a society_member and do the checks
  societyMemberChecks cid bob alice chuck
  -- Let's promote bob
  WS.bracketR3 c alice bob chuck $ \(wsA, wsB, wsC) -> do
    let updateUp = OtherMemberUpdate (Just roleNameSocietyAdmin)
    ...
    void . liftIO $ WS.assertMatchN (5 # Second) [wsA, wsB, wsC] $ do
      wsAssertMemberUpdateWithRole cid alice bob roleNameSocietyAdmin
  societyAdminChecks cid bob alice chuck
...
societyAdminChecks ::
  ...
  UserId ->
  TestM ()
societyAdminChecks cid admin otherAdmin mem = do
  let role = roleNameSocietyAdmin
  other <- randomUser
...
societyMemberChecks ::
  ...
  UserId ->
  TestM ()
societyMemberChecks cid mem admin otherMem = do
  let role = roleNameSocietyMember
  other <- randomUser
