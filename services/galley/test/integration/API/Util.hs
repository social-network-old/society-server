...
import Web.Cookie
import qualified Society.API.Message.Proto as Proto
...
createTeamConv u tid us name acc mtimer = createTeamConvAccess u tid us name acc Nothing mtimer (Just roleNameSocietyAdmin)
...
createTeamConvAccessRaw u tid us name acc role mtimer convRole = do
  ...
  let conv =
        NewConvUnmanaged $
          NewConv (makeIdOpaque <$> us) name (fromMaybe (Set.fromList []) acc) role (Just tinfo) mtimer Nothing (fromMaybe roleNameSocietyAdmin convRole)
  post
    ( g
        . path "/conversations"
        . zUser u
...
createManagedConv u tid us name acc mtimer = do
  ...
  let conv =
        NewConvManaged $
          NewConv (makeIdOpaque <$> us) name (fromMaybe (Set.fromList []) acc) Nothing (Just tinfo) mtimer Nothing roleNameSocietyAdmin
  r <-
    post
      ( g
...
createOne2OneTeamConv u1 u2 n tid = do
  ...
  let conv =
        NewConvUnmanaged $
          NewConv [makeIdOpaque u2] n mempty Nothing (Just $ ConvTeamInfo tid False) Nothing Nothing roleNameSocietyAdmin
...
postConv u us name a r mtimer = postConvWithRole u us name a r mtimer roleNameSocietyAdmin
...
postConvWithReceipt u us name a r mtimer rcpt = do
  g <- view tsGalley
  let conv = NewConvUnmanaged $ NewConv (makeIdOpaque <$> us) name (Set.fromList a) r Nothing mtimer (Just rcpt) roleNameSocietyAdmin
  post $ g . path "/conversations" . zUser u . zConn "conn" . zType "access" . json conv
...
postO2OConv u1 u2 n = do
  g <- view tsGalley
  let conv = NewConvUnmanaged $ NewConv [makeIdOpaque u2] n mempty Nothing Nothing Nothing Nothing roleNameSocietyAdmin
  post $ g . path "/conversations/one2one" . zUser u1 . zConn "conn" . zType "access" . json conv
...
assertConv ::
  ...
  Maybe Milliseconds ->
  TestM ConvId
assertConv r t c s us n mt = assertConvWithRole r t c s us n mt roleNameSocietyAdmin
...
assertConvWithRole r t c s us n mt role = do
  ...
  liftIO $ do
    ...
    assertEqual "creator is always and admin" (Just roleNameSocietyAdmin) (memConvRoleName <$> _self)
...
wsAssertMemberJoin conv usr new n = wsAssertMemberJoinWithRole conv usr new roleNameSocietyAdmin n
