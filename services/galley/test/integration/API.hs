...
postJoinConvOk = do
  ...
  WS.bracketR2 c alice bob $ \(wsA, wsB) -> do
    ...
    void . liftIO $ WS.assertMatchN (5 # Second) [wsA, wsB] $
      wsAssertMemberJoinWithRole conv bob [bob] roleNameSocietyMember
...
postConvertTeamConv = do
  ...
  WS.bracketR3 c alice bob eve $ \(wsA, wsB, wsE) -> do
    ...
    void . liftIO $ WS.assertMatchN (5 # Second) [wsA, wsB, wsE] $
      wsAssertMemberJoinWithRole conv mallory [mallory] roleNameSocietyMember
...
postConvO2OFailWithSelf = do
  g <- view tsGalley
  alice <- randomUser
  let inv = NewConvUnmanaged (NewConv [makeIdOpaque alice] Nothing mempty Nothing Nothing Nothing Nothing roleNameSocietyAdmin)
  ...
  let memberBob =
        Member
          { memId = bob,
            ...
            memConvRoleName = fromMaybe roleNameSocietyAdmin (mupConvRoleName update)
          }
...
removeUser = do
  ...
  liftIO $ do
    ...
    (mems2 >>= other carl) @?= Just (OtherMember carl Nothing roleNameSocietyAdmin)
    (mems3 >>= other bob) @?= Nothing
    (mems3 >>= other carl) @?= Just (OtherMember carl Nothing roleNameSocietyAdmin)
  where
