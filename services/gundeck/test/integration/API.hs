...
testRegisterPushToken = do
  ...
  t12 <- randomToken c1 apnsToken {tName = (AppName "com.society.ent")} -- different app
...
testNoPingNoPong = do
  ...
  [(_, [(chread, chwrite)] :: [(TChan ByteString, TChan ByteString)])] <-
    connectUsersAndDevicesWithSendingClients ca [(uid, [connid])]
  liftIO $ do
    atomically $ writeTChan chwrite "Society is so much nicer with internet!"
    msg <- waitForMessage chread
...
testSharePushToken = do
  ...
  let tok2 = pushToken APNSVoIP "com.society.dev.ent" apsTok
  let tok3 = pushToken APNS "com.society.int.ent" apsTok
