...
- [list of howtos for supported SAML IdP vendors](https://docs.social.network/how-to/single-sign-on/index.html)
- [fragment](https://docs.social.network/understand/single-sign-on/design.html)  (TODO: clean up the section "common misconceptions" below and move it here.)
- [official docs for team admin from customer support](https://support.social.network/hc/en-us/categories/360000248538?section=administration%3Fsection%3Dadministration)  (skip to "Authentication")
- [talk scim using curl](https://github.com/social-network/society-server/blob/develop/docs/reference/provisioning/scim-via-curl.md)
- if you want to work on our saml/scim implementation and do not have access to [https://github.com/zinfra/backend-issues/issues?q=is%3Aissue+is%3Aopen+label%3Aspar] and [https://github.com/social-network/design-specs/tree/master/Single%20Sign%20On], please get in touch with us.
...
- `.../society-server/deploy/services-demo/register_idp_internal.sh`
- `${METADATA_FILE}`
...
the login code from the `id` field of that object by adding `society-` in
front, eg.:

```
society-e97fbe2e-eeb1-11e9-acf3-9ba77d8a04bf
```

give this login code to the users that you want to connect to society
using this idp.  see
[here](https://support.social.network/hc/en-us/articles/360000954617-Login-with-SSO)
...
completely different), these changes need to be updated in society.  We
...
# The ID of the IdP you want to update (login code without the `society-`
...
- If a user logs into the team for the first time using the old login
  ...
  and society only authenticates existing users with it.
...
- If you go to team settings, you will see the old IdP and the new
  ...
  https://github.com/social-network/society-team-settings/issues/3465).
...
Note the lack of the `society-` prefix.
...
- find metadata for team in table `spar.idp_raw_metadata` via cqlsh
  (since https://github.com/social-network/society-server/pull/872)
...
https://github.com/social-network/saml2-web-sso supports writing [unit vendor
compatibility
tests](https://github.com/social-network/saml2-web-sso/blob/ff9b9f445475809d1fa31ef7f2932caa0ed31613/test/Test/SAML2/WebSSO/APISpec.hs#L266-L329)
...
email address that society is sending emails to.
...
scim](https://github.com/social-network/society-server/blob/010ca7e460d13160b465de24dd3982a397f94c16/services/spar/test-integration/Test/Spar/Scim/UserSpec.hs#L239-L308);
[test case for
saml](https://github.com/social-network/society-server/blob/293518655d7bae60fbcb0c4aaa06034785bfb6fc/services/spar/test-integration/Test/Spar/APISpec.hs#L742-L795))

For the semantics of interesting corner cases, consult [the test
suite](https://github.com/social-network/society-server/blob/develop/services/spar/test-integration/Test/Spar/APISpec.hs).
...
[`ManagedBy`](https://github.com/social-network/society-server/blob/010ca7e460d13160b465de24dd3982a397f94c16/libs/brig-types/src/Brig/Types/Common.hs#L393-L413);
see also {#DevScimOneWaySync}.  This is a half-implemented feature for
managing conflicts between changes via scim vs. changes from society
clients; and does currently not affect deletability of users.


#### delete via deleting idp

[Currently](https://github.com/social-network/society-server/blob/010ca7e460d13160b465de24dd3982a397f94c16/services/spar/src/Spar/API.hs#L172-L187),
...
- if a new idp is registered, and a user authenticates via that idp, the old user is unreachable.  (spar will look up the society `UserId` under the saml user id that consists partly of the id of the new IdP, come up empty, and [create a fresh user on brig](https://github.com/social-network/society-server/blob/010ca7e460d13160b465de24dd3982a397f94c16/services/spar/src/Spar/App.hs#L306).)
