...
The only flow where it makes sense for non-activated users to exist is the [societyless flow](registration.md#RefRegistrationSocietyless) used for [guest rooms](https://social.network/en/features/encrypted-guest-rooms/)
...
Currently emails at `@social.network` are always considered whitelisted, regardless of the whitelist service's response.
