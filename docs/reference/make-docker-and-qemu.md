# About this document:
This document is written with the goal of explaining https://github.com/social-network/society-server/pull/622 well enough that someone can honestly review it. :)
...
To answer that, we're going to have to go back to Society-Server, specifically, our integration tests. Integration tests are run locally on all of our machines, in order to ensure that changes we make to the Society backend do not break currently existing functionality. In order to simulate the components that society's backend depends on (s3, cassandra, redis, etc..), we use a series of docker images. These docker images are downloaded from dockerhub, are maintained (or not maintained) by outside parties, and are built by those parties.
...
$ cat Dockerfile | sed "s/^\(MAINTAINER\).*/\1 Julia Longtin \"julia.longtin@social.network\"/" | sed "s=^\(FROM \)\(.*\)$=\1i386/\2=" > Dockerfile-386
$ cat Dockerfile | sed "s/^\(MAINTAINER\).*/\1 Julia Longtin \"julia.longtin@social.network\"/" | sed "s=^\(FROM \)\(.*\)$=\1\2=" > Dockerfile-amd64
...
```bash
make DIST=DEBIAN DOCKER_USERNAME=julialongtin DOCKER_EMAIL=julia.longtin@social.network DOCKER_REALNAME='Julia Longtin' build-all -j".
```
...
```bash
$ cat Makefile | grep "?="
DOCKER_USERNAME ?= societyserver
DOCKER_REALNAME ?= Society
DOCKER_EMAIL    ?= backend@social.network
TAGNAME         ?= :0.0.9
DIST            ?= DEBIAN
...
The '?=' assignment operator is used to provide a default value. When earlier, we ran make as "make DIST=DEBIAN DOCKER_USERNAME=julialongtin DOCKER_EMAIL=julia.longtin@social.network DOCKER_REALNAME='Julia Longtin' build-all -j", we were overriding those values. the Make interpreter will use values provided on the command line, or values we have used 'export' to place into our shell environment.
