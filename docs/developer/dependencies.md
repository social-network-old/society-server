...
This page documents how to install necessary dependencies to work with the society-server code base.
...
_Note_: Debian is not recommended due to this issue when running local integration tests: [#327](https://github.com/social-network/society-server/issues/327)*. This issue does not occur with Ubuntu.
...
_Note_: If you want to build debian-style packages of cryptobox-c and other society utilities, execute this step. otherwise, make sure to execute the 'Generic' version of the cryptobox-c step.

```bash
git clone https://github.com/social-network/society-server && cd society-server/tools/makedeb
export VERSION=0
make dist
...
```bash
git clone https://github.com/social-network/cryptobox-c && cd cryptobox-c
make dist
...
### Generic
```bash
export TARGET_LIB="$HOME/.society-dev/lib"
export TARGET_INCLUDE="$HOME/.society-dev/include"
mkdir -p "$TARGET_LIB"
...
extra-include-dirs:
- /usr/local/include
- <YOUR_HOME_DIR>/.society-dev/include

extra-lib-dirs:
- /usr/local/lib
- <YOUR_HOME_DIR>/.society-dev/lib
```
