...
module Society.API.Message.Proto
  ( UserId,
    userId,
    fromUserId,
    ClientId,
...
import Imports
import qualified Society.API.Message as Msg
import qualified Society.API.User.Client as Client
...
instance Enum Priority where
  ...
  toEnum x = error $ "Society.API.Message.Proto.Priority: invalid enum value: " ++ show x
