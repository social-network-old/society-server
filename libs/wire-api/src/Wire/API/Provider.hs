...
--
-- FUTUREWORK: Deduplicate with 'Society.API.User'?
module Society.API.Provider
  ( -- * Provider
    Provider (..),
    ProviderProfile (..),
...
import Society.API.Provider.Service (ServiceToken (..))
import Society.API.Provider.Service.Tag (ServiceTag (..))
import Society.API.User.Identity (Email)
import Society.API.User.Profile (Name)
