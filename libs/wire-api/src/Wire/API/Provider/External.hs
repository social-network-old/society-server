...
module Society.API.Provider.External
  ( NewBotRequest (..),
    NewBotResponse (..),
  )
where
...
import Imports
import Society.API.Provider.Bot (BotConvView, BotUserView)
import Society.API.User.Client.Prekey (LastPrekey, Prekey)
import Society.API.User.Profile (Asset, ColourId, Locale, Name)
