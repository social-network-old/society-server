...
module Society.API.Event.Conversation
  ( -- * Event
    Event (..),
    EventType (..),
    EventData (..),
...
import Society.API.Conversation (modelConversationAccessUpdate, modelConversationMessageTimerUpdate, modelConversationReceiptModeUpdate, modelConversationUpdateName)
import Society.API.Conversation
import Society.API.Conversation.Code (modelConversationCode)
import qualified Society.API.Conversation.Code as Code
import Society.API.Conversation.Member
import Society.API.Conversation.Role
import Society.API.Conversation.Typing (modelTyping)
import Society.API.Conversation.Typing (TypingData)
import Society.API.User (UserIdList)
  ...
  parseJSON = withObject "simple-members-payload" $ \o -> do
    ...
    membs <- case users of
      ...
      Nothing -> do
        ids <- o .:? "user_ids"
        case ids of
          Just userIds -> pure $ fmap (\u -> SimpleMember u roleNameSocietyAdmin) userIds
...
instance FromJSON SimpleMember where
  parseJSON = withObject "simple member object" $ \o ->
    SimpleMember <$> o .: "id"
      <*> o .:? "conversation_role" .!= roleNameSocietyAdmin
