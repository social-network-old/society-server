...
import Imports
import Society.API.Provider.Service (ServiceRef, modelServiceRef)
import Society.API.Team (BindingNewTeam, modelNewBindingTeam)
import Society.API.User.Activation (ActivationCode)
import Society.API.User.Auth (CookieLabel)
...
instance FromJSON NewUserPublic where
  parseJSON val = do
    ...
    when (newUserManagedBy nu `notElem` [Nothing, Just ManagedBySociety]) $
      fail "only managed-by-Society users can be created here."
