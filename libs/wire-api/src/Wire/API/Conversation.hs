...
module Society.API.Conversation
  ( -- * Conversation
    Conversation (..),
    ConversationList (..),
...
import Society.API.Conversation.Role (RoleName, roleNameSocietyAdmin)
...
newConvParseJSON = withObject "new-conv object" $ \i ->
  NewConv <$> i .: "users"
    ...
    <*> i .:? "message_timer"
    <*> i .:? "receipt_mode"
    <*> i .:? "conversation_role" .!= roleNameSocietyAdmin
...
newInvite us = Invite us roleNameSocietyAdmin
...
instance FromJSON Invite where
  parseJSON = withObject "invite object" $ \o ->
    Invite <$> o .: "users" <*> o .:? "conversation_role" .!= roleNameSocietyAdmin
