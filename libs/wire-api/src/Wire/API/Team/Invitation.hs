...
module Society.API.Team.Invitation
  ( InvitationRequest (..),
    Invitation (..),
    InvitationList (..),
...
import Imports
import Society.API.Team.Role (Role, defaultRole, typeRole)
import Society.API.User.Identity (Email, Phone)
import Society.API.User.Profile (Locale, Name)
...
modelTeamInvitationRequest = Doc.defineModel "TeamInvitationRequest" $ do
  Doc.description "A request to join a team on Society."
...
modelTeamInvitation = Doc.defineModel "TeamInvitation" $ do
  Doc.description "An invitation to join a team on Society"
