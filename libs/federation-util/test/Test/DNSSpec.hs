...
spec = do
  ...
  describe "srvLookup" $ do
    it "returns the expected result for social.network" $ do
      rs <- makeResolvSeed defaultResolvConf
      society <- srvLookup'' mockLookupSRV "_society-server" "social.network" rs
      society `shouldBe` Just [("social.network", 443)]
    it "filters out single '.' results" $ do
      rs <- makeResolvSeed defaultResolvConf
      exampleDotCom <- srvLookup'' mockLookupSRV "_society-server" "example.com" rs
      exampleDotCom `shouldBe` Nothing
    it "can return multiple results" $ do
      rs <- makeResolvSeed defaultResolvConf
      zinfra <- srvLookup'' mockLookupSRV "_society-server" "zinfra.io" rs
      (length <$> zinfra) `shouldBe` Just 2
    it "returns Nothing if there is no DNS record" $ do
      rs <- makeResolvSeed defaultResolvConf
      noRecord <- srvLookup'' mockLookupSRV "_society-server" "no-record-here" rs
...
mockLookupSRV _ domain = do
  case domain of
    "_society-server._tcp.social.network." -> return $ Right [(0, 0, 443, "social.network")]
    "_society-server._tcp.zinfra.io." -> return $ Right [(0, 0, 443, "server1.zinfra.io"), (0, 0, 443, "server2.zinfra.io")]
    "_society-server._tcp.example.com." -> return $ Right [(0, 0, 443, ".")]
    _ -> return $ Right []
