...
module Network.Society.Client.API.Conversation
  ( postOtrMessage,
    createConv,
    getConv,
    addMembers,
...
import Network.Society.Client.API.Push (ConvEvent)
import Network.Society.Client.HTTP
import Network.Society.Client.Monad (ClientException (ParseError))
import Network.Society.Client.Session
  ...
  where
    req =
      method POST
        . path "conversations"
        . acceptJson
        . json (NewConvUnmanaged (NewConv users name mempty Nothing Nothing Nothing Nothing roleNameSocietyAdmin))
        $ empty
    rsc = status201 :| []
