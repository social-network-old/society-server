...
module Network.Society.Bot.Crypto
  ( Plaintext,
    Ciphertext,
...
import Imports
import Network.Society.Bot.Clients
import Network.Society.Bot.Crypto.Glue (randomBytes, unwrap)
import Network.Society.Bot.Monad
import Network.Society.Client.API.Client
import Network.Society.Client.API.Conversation
...
aesIV = fromMaybe (error "Network.Society.Bot.Crypto: Bad IV") . makeIV

padPKCS7 :: Plaintext -> Plaintext
padPKCS7 = pad (PKCS7 16)

unpadPKCS7 :: Plaintext -> Plaintext
unpadPKCS7 = fromMaybe (error "Network.Society.Bot.Crypto: Bad padding") . unpad (PKCS7 16)
