...
import Util.Options
import qualified Society.Swagger as Doc
...
routes = do
  ...
  document "POST" "revokeIdentity" $ do
    ...
    Doc.notes
      ...
      \deactivated (\"societyless\") and might thus become inaccessible. \
  ...
  -- The following endpoint are only relevant internally at Society
  ...
  document "GET" "getTeamInvoice" $ do
    ...
    notes "Relevant only internally at Society"
    Doc.parameter Doc.Path "tid" Doc.bytes' $
