# How to run `society-server` with "fake" AWS dependencies in demo mode
...
# limit of docker. More on https://github.com/social-network/society-server/issues/326
...
It is however very straightforward to setup all the necessary dependencies to run `society-server` and it is what we use in our integration tests as well (as can be seen in our [integration bash script](../../services/integration.sh)).
...
```
# from the root of society-server directory
...
Next, from the society-server directory, after having compiled everything with 'make install':
...
./dist/api-smoketest \
    ...
    --sender-email=backend-demo@mail.demo.social.network \
